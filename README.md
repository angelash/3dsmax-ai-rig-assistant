...
| `server/stage01_skin_prep_gate.py` | 汇总 Stage01 QC，生成进入 Skin 前的 MDC 本地代理语义确认和权重准备门 |
...
7. 由 MDC 本地代理检查 Figure Mode 下的 Biped 匹配。
...
`visual_review_pack.py` 会在 run 内生成 `visual_review/`：`full/` 保存全局前/侧/顶证据图，`regions/` 保存 head、pelvis、left/right hand、left/right foot 的局部裁剪，`review_input.md` 和 `review_schema.json` 用于 MDC 本地代理做结构化语义审查。它只输出 blocker/pass/uncertain 这类审查项，不输出分数。硬规则是：frontWrap、sideWrap、topWrap、rootPelvisPolicy、legClothingOcclusion、footPivot 等必要检查没有全部 `pass`，不得进入 Skin。规则细节见 `docs/stage01-biped-multiview-signoff.md`。

`batch_stage01_fbx.ps1` 会在证据包生成后执行最终 Skin gate：如果传入 `-VisualSignoffJson`，使用该本地签核；如果没有签核，gate 会保持阻断并说明缺少多视图包裹性确认。工程不再包含任何外部视觉 API 调用，也不需要 API key；视觉识别由 MDC 本地代理查看证据图后填写 `visual_review/semantic_visual_review_template.json`，最终仍由 `stage01_skin_prep_gate.py` 校验 schema、必填检查项、Biped fit 和资产 QC。
...
- MDC 本地代理对 front / side / top 包裹性的结构化签核。
- MDC 本地代理语义确认、Skin、权重和变形测试。
