...
- `semanticSkinReady`：Biped 贴合诊断、`semanticPolicyReady` 和 `multiviewWrapConfirmed` 同时为 true。只靠脚本生成、单项数值贴合或不完整的 VLM JSON，不能让它变 true。
...
当前自动批处理会在证据包生成后重跑最终 Skin gate：有 `-VisualSignoffJson` 时使用外部签核；有 `OPENAI_API_KEY` 且未传 `-SkipVlmReview` 时调用 VLM 读取 front/side/top、纹理对照和局部裁剪来生成签核 JSON；没有签核时保持阻断。即使旧 JSON 字段里保留了诊断分数，只要签核缺失、schema 不完整、任何必要项不是 `pass`，`semanticSkinReady`、`stage01HandoffReady`、`skinSetupReady` 和 `productionReady` 仍会保持 false。报告会列出人工/VLM 清单：front/side/top 包裹性、身体中心链、腿/脚 pivot、锁骨/肩肘腕/手团中心线、短脖子大头、以及需要延后的冠饰、喙、布料、武器、翅膀或手指细节骨。
