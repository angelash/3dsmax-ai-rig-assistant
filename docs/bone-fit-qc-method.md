...
> 当前决策口径：旧的算法评分、benchmark 排序和“推荐算法”已经停用。分数只允许作为历史诊断字段保留，不能用于 Skin 放行或生产交付。Stage01 现在只输出 `tutorial_centerline_qbird` 视觉候选，用它来模拟 MDC 按教程看图校准；实际被编辑和进入 Skin 准备的骨架只能是 Biped。
...
- 如果还没有 MDC 确认语义关节点，`stage01HandoffReady = false`。
...
- `multiviewWrapConfirmed`：MDC 本地代理已按 `visual_review/review_schema.json` 审查 front/side/top 包裹性，并且必要项全部 `pass`。
- `semanticSkinReady`：Biped 贴合诊断、`semanticPolicyReady` 和 `multiviewWrapConfirmed` 同时为 true。只靠脚本生成、单项数值贴合或不完整的签核 JSON，不能让它变 true。
- `stage01HandoffReady`：在候选可用、语义阻塞项解决、MDC 确认完成后，才允许进入 Skin 准备。
...
这一步能把“看起来是否明显错位”变成可复查的截图和问题清单，也能给后续 MDC 本地代理复核提供稳定输入。它仍不能替代 MDC 对语义关节点的确认。
...
- `slices/`：骨盆-脊柱、膝-踝、踝-趾这类局部骨段包裹性截图。Biped 的 3 段腿结构里实际脚段是 `Ankle -> Toe`；`L_Foot/R_Foot` 只作为视觉脚掌语义参考，不映射到第二套 Bones。
- `review_input.md`：MDC 本地代理需要查看的证据和问题。
...
Skin 前置门会把 `visual_review/semantic_visual_review_template.json` 或同 schema 的本地签核输出作为硬输入。必要检查包括 `frontWrap`、`sideWrap`、`topWrap`、`rootPelvisPolicy`、`crossSectionInsideVolume`、`legClothingOcclusion`、左右手和左右脚 pivot。缺失、不确定、needs_detail 或 blocker 都会阻断 Stage01 handoff。

外部视觉 API 自动复核器已移除。`visual_review/*_visual_evidence_manifest.json`、`review_input.md` 和 `review_schema.json` 只作为 MDC 本地代理读图的输入；最终仍由 `stage01_skin_prep_gate.py` 校验完整 schema、推荐值、必填检查项和 Biped fit。
...
Biped 的位置设置会受到 IK/层级约束影响，所以自动产线不能只相信写入坐标。正确做法是像教程一样反复用前/侧/顶视图检查，再用视觉截图和 Semantic Skin Review 做 MDC 签核。
...
- 在 Max 里由 MDC 调整 Guide 后重新拟合 Biped 和 QC。
...
- 增加截图/视图辅助校验，让 MDC 按前、侧、顶视图检查 Guide。

### 长期

- 用 MDC 本地代理或 3D landmark 模型参与 Guide 标注，但仍只让它调用白名单工具。
- 对标准人形/动物可以研究 RigNet、UniRig 类自动 rigging；对 A1 这种 Q 版禽鸟三国角色，建议作为辅助参考，不作为直接交付结果。
- 训练或沉淀项目内角色模板和失败案例，让 MDC 更像绑定师一样判断 Biped 结构参数和语义关键点。
