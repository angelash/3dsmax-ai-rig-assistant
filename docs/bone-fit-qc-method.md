...
> 当前决策口径：旧的算法评分、benchmark 排序和“推荐算法”已经停用。分数只允许作为历史诊断字段保留，不能用于 Skin 放行或生产交付。Stage01 现在只输出 `tutorial_centerline_qbird` 视觉候选，再由前/侧/顶截图、Semantic Skin Review 和人工语义确认决定下一步。
...
4. 检查躯干侧面姿态和大头短脖子。
5. 检查锁骨、肩、肘、腕、手团。
6. 检查每段模板骨的位置、长度比例、方向规则、显示粗细和左右镜像。
...
- 如果视觉候选缺少模板骨骼、截图或语义复核数据，`stage01CandidateAvailable = false`。
...
- `stage01CandidateAvailable`：模板骨骼、视觉截图和逐骨/语义报告都已生成，说明有候选可看。
...
- MaxScript 导出 `*_visual_snapshot.json`，包含 mesh 点云、Guide、模板骨骼和包围盒。
...
这一步仍不产生分数，只服务于语义 blocker 判断：Root/Pelvis 是否只做控制轴、HeadTop 是否是冠饰、手部是否需要 detail bone、脚掌 pivot 是否能从 side/top 视图确认。

## 陆逊模型当前结论

当前查看 `luxun_model_tutorial_centerline_qbird` 最新 run，场景中包含两套骨骼对象：

- `AIRA_Biped_COM`：Biped 参考骨架。
- `AIRA_BONE_*`：从 Guide 直接生成的模板骨骼链。

旧的七算法 benchmark 结果已经停用，不再作为“推荐”或“排名”。`tutorial_centerline_qbird` 现在只是唯一视觉候选生成器；它生成的模板骨骼需要通过截图和 Semantic Skin Review 检查后，才能进入人工语义签核。

当前关键结论不是分数，而是阻塞项：

- Root->Pelvis 需要明确为 control-only non-deforming，不应直接进 Skin 权重。
- HeadTop 可能被冠/头饰极值拉高，需要确认是头骨体积还是饰件。
- 单块手部质量需要确认是否要拆手指/爪/武器 detail。
- 脚掌/Toe pivot 必须用 side/top 视图签核。
- 原模型仍有贴图、Skin、权重和变形测试未完成。

Biped 不合格的原因是 3ds Max Biped 的位置设置会受到 IK/层级约束影响。Autodesk 文档也说明，设置 Biped 位置时最终位置可能被 Biped IK 钳制。因此自动产线不应该把 Biped 位置写入当成强保证。

## 改进方向

### 短期

- 保留 Biped 作为人工教程和参考骨架。
- 自动产线优先使用 `AIRA_BONE_*` 模板骨骼，因为普通骨骼能精确遵守 Guide。
- 在 Max 里人工调整 Guide 后重新跑模板骨骼和 QC。
...
- 训练或沉淀项目内角色模板和失败案例，让 AI 先判断“适合模板骨架”还是“需要人工关键点”。
