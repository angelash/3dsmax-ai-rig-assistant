...
- 参考答案只能用于离线评测、复盘和提炼通用规则；无参考自动绑骨流程不得把参考 FBX/JSON 当作必要输入。
...
- 离线对照 AccuRig 参考答案骨点、骨段长度、脚/手/头语义，用来发现通用问题；实际修复策略必须能在没有参考答案的新模型上运行。
...
来自 2026-05-23 A1 全量参考复盘：

- 最新 CT refine 结果达到严格 CT `311 -> 20`、role-aware CT `15`，但这只说明 Biped 中心线通常被模型体积包住，不等同于 AccuRig 参考答案级骨点/Skin。
- 参考 FBX 骨点只作为离线监督信号：全量主关节点平均误差约 `9.969%` 身高，主要集中在 Toe、Ankle、Knee、Wrist。
- 直接把候选 Biped snap 到参考 AccuRig 主骨点会让 CT 从 `20/15` 恶化到 `304/246`，说明 AccuRig pivot 和 Biped 段端点不能一对一硬套。
- 无参考 anatomy-safe nudge 可以保留：它只用点云、身高比例、局部脚/手点云和 CT 不退护栏，不读取参考答案；A1 离线验收从平均参考误差 `9.969%` 降到 `9.150%`，P90 从 `18.616%` 降到 `16.248%`，CT 保持 `20/15` 不退。
- 下一步应把有效的无参考规则继续收敛到主流程：脚踝/脚趾贴近地面和脚云，异常高膝盖回隐藏腿轴，偏低手腕回前臂末端高度；参考答案只用于回归评分。
